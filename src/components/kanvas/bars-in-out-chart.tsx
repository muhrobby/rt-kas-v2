...
export function BarsInOutChart({
  ...
  return (
    <div>
      <div className="flex items-end border-b border-kanvas-line px-1" style={{ gap, height }}>
        {data.map((entry) => (
      ...
      <div className="flex px-1 pt-1.5" style={{ gap }}>
        {data.map((entry) => (
          <div key={`axis-${entry.bulan}`} className="flex-1 text-center text-[10px] font-medium tracking-[0.4px] text-kanvas-ink-4 uppercase">
            {entry.bulan}
          </div>
        ))}
      </div>
