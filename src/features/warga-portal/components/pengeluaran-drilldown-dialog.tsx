      ...
      <div className="p-4 sm:p-6">
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <h2 className="mt-1 text-[22px] text-kanvas-ink">{monthLabel}</h2>
          </div>
          <button type="button" onClick={onClose} className="rounded p-1 text-kanvas-ink-3" aria-label="Tutup rincian pengeluaran">
        ...
        <div className="space-y-2">
          {rows.length > 0 ? rows.map((row) => (
            <div key={`${monthLabel}-${row.kategori}`} className="flex flex-col gap-1 rounded-lg border border-kanvas-line bg-kanvas-paper-2 px-3 py-2.5 text-[12px] sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="font-semibold text-kanvas-ink">{formatRupiah(row.nominal)}</span>
            </div>
          )) : (
            <div className="rounded-lg border border-dashed border-kanvas-line bg-kanvas-paper-2 px-3 py-2.5 text-[12px] text-kanvas-ink-3">
        ...
        <div className="mt-3.5 flex items-center justify-between rounded-lg border border-kanvas-line bg-white px-3 py-2.5">
          ...
          <p className="text-[14px] font-semibold text-kanvas-ink">{formatRupiah(total)}</p>
        </div>
