  ...
  return (
    <AppModal open={open} onClose={onClose} width={460}>
      <div className="p-4 sm:p-6">
        ...
        <div className="rounded-lg border border-dashed border-kanvas-line bg-white p-4">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-[11px] font-semibold tracking-[0.6px] text-kanvas-ink-4 uppercase">Nomor</p>
            <p className="text-[12px] font-semibold text-kanvas-ink">{data?.refKuitansi ?? "-"}</p>
          </div>
          <div className="grid grid-cols-[110px_1fr] gap-y-2 text-[12px] text-kanvas-ink-3">
            <span>Diterima dari</span><span className="font-semibold text-kanvas-ink">{data?.wargaNama ?? "-"}</span>
            <span>Blok</span><span>{data?.blok ?? "-"}</span>
            <span>Untuk</span><span className="font-semibold text-kanvas-ink">{data?.kategori ?? "-"}</span>
          ...
          <div className="mt-4 border-t border-kanvas-line-2 pt-3">
            <p className="text-[11px] font-semibold tracking-[0.6px] text-kanvas-ink-4 uppercase">Jumlah</p>
            <p className="mt-1 text-[28px] leading-none text-kanvas-ink">{formatRupiah(data?.nominal ?? 0)}</p>
          </div>
        </div>
