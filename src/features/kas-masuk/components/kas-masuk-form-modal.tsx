    ...
    <AppModal open={open} onClose={onClose} width={600}>
      <div className="p-4 sm:p-6">
        <div className="mb-4 flex items-start justify-between gap-2">
          <div>
            <p className="text-[11px] font-semibold tracking-[0.7px] text-[var(--kanvas-ink-4)] uppercase">Kas Masuk</p>
            <h2 className="mt-1 text-2xl text-[var(--kanvas-ink)]">Input Pembayaran Iuran</h2>
          </div>
          <button type="button" onClick={onClose} className="rounded p-1 text-[var(--kanvas-ink-3)]" aria-label="Tutup modal kas masuk">
        ...
        <AppField label="Warga">
          ...
          {errors.wargaId || fieldErrors?.wargaId?.[0] ? (
            <p className="mt-1 text-[11px] text-[var(--kanvas-danger)]">{errors.wargaId || fieldErrors?.wargaId?.[0]}</p>
          ) : null}
        </AppField>
            ...
            renderItem={(option) => {
              ...
              return (
                <div className="flex items-start justify-between gap-2">
                  ...
                  <p className="text-[12px] text-[var(--kanvas-ink-3)]">{formatRupiah(item.nominal)}</p>
                </div>
              )
            }}
          />
          ...
          <>
            <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2 md:gap-3">
              ...
              <AppField label="Bulan Dipilih">
                <div className="rounded-lg border border-[var(--kanvas-line)] bg-white px-3 py-2.5 text-[13px] text-[var(--kanvas-ink)]">
            ...
            <AppField label="Selector Bulan" hint="Bulan tercoret = sudah dibayar">
              {loadingPaidMonths ? (
                <p className="text-[11px] text-[var(--kanvas-ink-3)]">Memuat...</p>
              ) : (
            ...
            {selectedKategori?.tipeTagihan === "bulanan" && values.bulan.length === 0 ? (
              <p className="-mt-1 text-[11px] text-[var(--kanvas-danger)]">Pilih minimal 1 bulan.</p>
            ) : null}
          </>
        ) : null}

        {selectedKategori?.tipeTagihan === "sekali" ? (
          <div className="mb-3 rounded-lg border border-[var(--kanvas-line)] bg-[var(--kanvas-paper)] p-2.5 text-[12px] text-[var(--kanvas-ink-3)]">
