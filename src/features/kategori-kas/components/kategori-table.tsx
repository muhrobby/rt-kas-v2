  ...
  return (
    <AppCard className="overflow-hidden p-0">
      <div className="lg:hidden">
        {kategori.length > 0 ? (
          kategori.map((item) => (
            <div key={item.id} className="border-b border-kanvas-line-2 px-4 py-3 last:border-b-0">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold text-kanvas-ink">{item.nama}</p>
                  <p className="text-[10.5px] text-kanvas-ink-4">{item.id}</p>
                </div>
                <p className="text-right text-[13px] font-semibold text-kanvas-ink">{formatRupiah(item.nominalDefault)}</p>
              </div>
              ...
              <div className="mt-3 flex flex-wrap justify-end gap-2">
                <button
                  type="button"
                  onClick={() => onEdit(item)}
                  className="inline-flex items-center gap-1 rounded-md border border-kanvas-line bg-white px-2.5 py-1.5 text-xs font-semibold text-kanvas-ink-2"
          ...
          <div className="px-4 py-10 text-center text-[13px] text-kanvas-ink-4">Tidak ada kategori ditemukan</div>
        )}
      </div>
      ...
      <div className="hidden lg:block">
        <div className="overflow-x-auto">
          <div className="min-w-[860px]">
            <div className="grid grid-cols-[1.6fr_0.8fr_0.75fr_1fr_0.9fr_90px] border-b border-kanvas-line bg-kanvas-paper px-4 py-2.5 text-[10px] font-bold tracking-[0.7px] text-kanvas-ink-3 uppercase">
            ...
            <div>
              {kategori.length > 0 ? (
                kategori.map((item) => (
                  <div
                    key={item.id}
                    className="grid grid-cols-[1.6fr_0.8fr_0.75fr_1fr_0.9fr_90px] items-center border-b border-kanvas-line-2 px-4 py-2.5 text-[13px] text-kanvas-ink-2"
                  >
                    <div>
                      <p className="font-semibold text-kanvas-ink">{item.nama}</p>
                      <p className="text-[10.5px] text-kanvas-ink-4">{item.id}</p>
